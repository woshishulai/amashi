...
Page({
    ...
    wx.request({
      ...
      success: function (res) { //这里写调用接口成功之后所运行的函数
        ...
        that.setData({
          ...
          consultant: res.data.data.p_data[0].employeeName,
  ...
  getReser() {
    let that = this
    console.log(that.data.userInfo)
    console.log(that.data.userInfo.patientID)
    if (that.data.userInfo.patientID == '' || that.data.userInfo.patientID == "null" || that.data.userInfo.patientID == "undefined" || that.data.userInfo.patientID == null) {
      wx.showModal({
        title: '温馨提示',
        content: '当前用户并未建档，请完成用户信息填写后进行预约',
        // showCancel: false,
        complete: (res) => {
          if (res.cancel) {
            // console.log('用户点击取消')
